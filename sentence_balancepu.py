...
def valid_instance_threshold(sentence_model,valid_sentence_dataloader,device,valid_y,V_ind,threshold,loss_func):
    sentence_model.eval()
    sentence_preds = []
    sentence_probability=[]
    scores=[]
    for batch in valid_sentence_dataloader:
        ...
        with torch.no_grad():
            outputs = sentence_model(**encoded_texts)
            # sentence_preds.extend(torch.argmax(outputs.logits, dim=1).cpu().numpy().tolist())
            sentence_probability.extend(loss_func(-outputs.logits.squeeze()).cpu().numpy().tolist())
            # print(((nn.Sigmoid()(outputs.logits[0:]))>0.5).to(torch.int).squeeze())
            # sentence_preds.extend(((nn.Sigmoid()(outputs.logits))>0.5).to(torch.int).squeeze().cpu().numpy().tolist())
    # indexed_list = list(enumerate(sentence_probability))
    # sorted_probability = sorted(indexed_list, key=lambda x: x[1])
...
if args.dataset in ['imdb','amazon','yelp','toxic','sms']:
    ...
    list_instance_imbalance=[int(i ) for i in [6,4,2] ]
...
for instance_imbalance in list_instance_imbalance:
    ...
    for seed in seeds:
        ...
        loss_func=(lambda x: nn.Sigmoid()(-x))# x 越大越接近1，loss越小
        # p=0.5
        for i in range(epoch):
            ...
            for idx,texts, labels in labeled_dataloader_PU:
                ...
                # if n_unlabeled== 0 or n_positive== 0:
                #     continue
                ...
                # loss=(loss_1+loss_2)/prior#*(instance_imbalance+1)
                # if torch.isnan(loss):
                #     print('前向Nan')
                #     print('logits:',logits)
                #     print('p:',p)
        ...
        # sorted_probability = sorted(loss_func(-pslab))
        ...
        if args.adj:
            ...
            threshold=0.5
        valid_instance_threshold(sentence_model,dataloader_T,device,test_y,ind_T,threshold,loss_func)
        #print('7')
