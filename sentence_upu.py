    ...
    for seed in seeds:
        ...
        for i in range(epoch):
            ...
            for idx,texts, labels in labeled_dataloader_PU:
                ...
                # loss_2 = nn.CrossEntropyLoss()(logits[unlabeled], labels[unlabeled])
                # loss_1 = nn.CrossEntropyLoss()(logits[positive], labels[positive])#+nn.CrossEntropyLoss()(logits[labels==1], 1-labels[labels==1])
                # epoch_pslab[idx] = logits.clone().detach()/torch.mean(logits,dim=0)

                # loss=2*prior*loss_1+loss_2
